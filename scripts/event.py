...
event_image = { trigger = { var:picture = 1 } texture = "gfx/event_pictures/paintings/cities/An_Overshot_Mill_in_Wales_James_Ward.dds" }
	...
	event_image = { trigger = { var:picture = 4 } texture = "gfx/event_pictures/paintings/cities/Thomas_Cole_Architect’s_Dream.dds" }
...
event = Template('''pdx{% if blank is not defined %}{% set blank = "" %}{% endif %}
{{namespace}}.{{eventid}} = {
	...
	{% else %}on_opened_soundeffect = "event:/SFX/Events/unspecific/gears_pistons"{% endif %}
	{{weight_multiplier}}{% if trigger != blank %}{{trigger}}
	{% else %}
	trigger = {
	
	}{% endif %}{% if immediate != blank %}{{immediate}}
	{% else %}
	immediate = {
...
class EventFile():
	...
	def make_file(self, args):
		for i, j in enumerate(args):
			if i == 0: j = f"namespace = {self.namespace}\n\n"
			self.file += j + "\n"
		self.write()

	def write(self, add_bom=True):
		path = f"output\\{self.namespace}.txt"
